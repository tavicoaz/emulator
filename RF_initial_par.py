...
def prepare_data(forcing_data, state_data):
    ...
    print(f"Combined dataframe (forcing and state data):\n{combined_df.head()}\n")
    ...
    print(f"Dataframe after adding previous day's state data:\n{combined_df[['Date', 'LEAFC', 'Prev_LEAFC', 'LEAFN', 'Prev_LEAFN']].head()}\n")

    # Drop rows with missing values (due to shift)
    print("Dropping rows with missing values...")
    final_df = combined_df.dropna()
    print(f"Final dataframe after dropping missing values:\n{final_df.head()}\n")

    return final_df


# Train a single model (to be run in parallel)
