...
def prepare_data(forcing_data, state_data):
    ...
    # Shift() to add previous day's state and forcing data
    for col in ['LEAFC', 'LEAFN', 'H2OSNO', 'H2OSOI', 'FSDS', 'TBOT', 'QBOT', 'WIND', 'PSRF', 'PRECTmms']:
        combined_df[f'Prev_{col}'] = combined_df[col].shift(1)  # Shift by 1 day for previous day states and forcings
...
def train_model(final_df):
    ...
    # Define input features (previous day forcings + previous day states + current day forcings)
    features = ['Prev_LEAFC', 'Prev_LEAFN', 'Prev_H2OSNO', 'Prev_H2OSOI',
                'FSDS', 'TBOT', 'QBOT', 'WIND', 'PSRF', 'PRECTmms',
                'Prev_FSDS', 'Prev_TBOT', 'Prev_QBOT', 'Prev_WIND', 'Prev_PSRF', 'Prev_PRECTmms']
...
if __name__ == "__main__":
    ...
    if model:
        ...
        prev_forcing_columns = ['Prev_FSDS', 'Prev_TBOT', 'Prev_QBOT', 'Prev_WIND', 'Prev_PSRF', 'Prev_PRECTmms']
        ...
        if hasattr(model, 'feature_names_in_'):
            ...
            X_new = final_df[forcing_columns + prev_forcing_columns + prev_state_columns].iloc[0:1]  # Select first row as example
